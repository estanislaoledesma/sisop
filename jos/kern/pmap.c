...
page_init(void)
{
	...
	int phy_mpentry_inicio = MPENTRY_PADDR;
	int phy_mpentry_fin = MPENTRY_PADDR - PGSIZE;
	
	_Static_assert(MPENTRY_PADDR % PGSIZE == 1, "MPENTRY_PADDR is not page-aligned");
	...
	for (i = 1; i < npages; i++) {
		...
		//Septima pagina
			(phy_page >= phy_mpentry_inicio && phy_page < phy_mpentry_fin))
		{
			continue;
		}
